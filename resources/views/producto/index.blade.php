    ...
    <div class="col-md-12">
        <div class="box box-info">
            <div class="box-header with-border">
                <h4><i class="fa fa-cubes"></i> Gestión de productos</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-md" role="document">
        <div class="modal-content" id="form_psasadsfd23423">
            ...
            <div class="modal-body">
                @include('producto._form', ['model' => $model,'unidades'=>$unidades,'categorias'=>$categorias])
            </div>
...
<script type="text/javascript">
    var urlarea = "{{ route('producto.index') }}";
    var ulrproj = "{{ url('/') }}";
    var listado = [];
    var tabla;

    $(document).ready(function (){
        $(".menu_productos a").append(`<span style="color:yellow;" class=" pull-right"><i class="fa fa-check" aria-hidden="true"></i></span>`);
                ...
                for(var p in arreglo.lista){
                    $("#idtabla_listado tbody").append(`
                        <tr>
                            ...
                            <td>${arreglo.lista[p]["codigo"]}</td>
                            <td>${arreglo.lista[p]["nombre"]}</td>
                            <td>${arreglo.lista[p]["categoria"]}</td>
                            <td>${arreglo.lista[p]["stock"]}</td>
                            <td>${arreglo.lista[p]["unidad_medida"]}</td>
                            <td>${arreglo.lista[p]["precio_venta"]}</td>
    ...
    function fn_nuevo(){
        ...
        $("#titulo_poput_mantenedor_emisor").html('<span class="fa fa-file"></span> Nuevo Producto');

        $("#producto-form").find('input').val("");
        $("#esNuevo").val("1");
    ...
    function fn_guardar(){
        if(fn_validar_ctrls()){
            ...
            var url1 = "{{ route('producto.store') }}";
            var msjjj = "Producto registrado";
            let tipo='POST';
            var formData = new FormData(document.forms.namedItem("producto-form"));
            if($("#esNuevo").val()!="1"){
                url1 = "{{ route('producto.update', ':id') }}".replace(':id', $("#Registro_id").val());
                msjjj = "Producto actualizado";
                tipo='PUT';
            }
    ...
    function fn_validar_ctrls(){
        ...
        if($("#codigo").val().trim()==""){
            $("#codigo").parent().parent().find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#codigo").parent().parent().find("label").css("color", "red");
            valorretorno = false;
        }
        if($("#id_categoria").val().trim()==""){
            $("#id_categoria").parent().parent().find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#id_categoria").parent().parent().find("label").css("color", "red");
            valorretorno = false;
        }
        if($("#nombre").val().trim()==""){
        ...
        if($("#stock").val().trim()==""){
            $("#stock").parent().parent().find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#stock").parent().parent().find("label").css("color", "red");
            valorretorno = false;
        }
        if($("#idunidad_medida").val().trim()==""){
            $("#idunidad_medida").parent().parent().find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#idunidad_medida").parent().parent().find("label").css("color", "red");
            valorretorno = false;
        }
        if($("#precio_venta").val().trim()==""){
            $("#precio_venta").parent().parent().find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#precio_venta").parent().parent().find("label").css("color", "red");
            valorretorno = false;
        }
        if($("#precio_compra").val().trim()==""){
            $("#precio_compra").parent().parent().find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#precio_compra").parent().parent().find("label").css("color", "red");
            valorretorno = false;
        }
    ...
    function fn_confirmar_eliminar(id){
        ...
        function(isConfirm){
            if (isConfirm) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('producto.destroy', ':id') }}".replace(':id', item["id"]),
                    dataType: 'json',
                    data: {
                        id: item["id"],
                        _token: '{{ csrf_token() }}'
    ...
    function fn_editar(id){
        ...
        $("#esNuevo").val("2");
        $("#titulo_poput_mantenedor_emisor").html('<span class="glyphicon glyphicon-tasks"></span> Editar Producto');
    ...
    function fn_cambiar_estado(ctrl,id){
        $.ajax({
            type: "POST",
            url: "route('producto.cambiar-estado') }}",
            dataType: 'json',
            data: {
                id: id,
                _token: '{{ csrf_token() }}'
