    ...
    <div class="col-md-12">
        <div class="box box-info">
            <div class="box-header with-border">
                <h4><i class="fa fa-file-text-o"></i> Gestión de Compras</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
        <div class="modal-content" id="form_psasadsfd23423">
            ...
            <div class="modal-body">
                @include('compra._form', ['model' => $model,'unidades'=>$unidades,'mediospago'=>$mediospago])
            </div>
...
<script type="text/javascript">
    var urlarea = "{{ route('compra.index') }}";
    var ulrproj = "{{ url('/') }}";
    var listado = [];
    var tabla;

    $(document).ready(function (){
        $(".menu_compras").addClass("active");
        $(".menu_compras a").append(`<span style="color:yellow;" class=" pull-right"><i class="fa fa-check" aria-hidden="true"></i></span>`);
    ...
    function fn_nuevo(){
        ...
        $("#titulo_poput_mantenedor_emisor").html('<span class="fa fa-file"></span> Nueva Compra');

        $("#compra-form").find('input,textarea').val("");
        $("#esNuevo").val("1");
        ...
        if(str.length==3){
            ...
            $("#Compra_fecha").datepicker("setDate", new Date(moment(d).format('YYYY'),moment(d).format('MM')-1,moment(d).format('DD')));
        }
    ...
    function fn_guardar(){
        if(fn_validar_ctrls()){
            ...
            var url1 = "{{ route('compra.store') }}";
            var msjjj = "Compra emitida";
            let tipo='POST';
            var formData = new FormData(document.forms.namedItem("compra-form"));
            if($("#esNuevo").val()!="1"){
                url1 = "{{ route('compra.update', ':id') }}".replace(':id', $("#Registro_id").val());
                msjjj = "Compra actualizada";
                tipo='PUT';
            }
    ...
    function fn_validar_ctrls(){
        ...
        if($("#Compra_fecha").val().trim()==""){
            $("#Compra_fecha").closest(".form-group").find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#Compra_fecha").closest(".form-group").find("label").css("color", "red");
            valorretorno = false;
        }
        if($("#Compra_serie").val().trim()==""){
            $("#Compra_serie").closest(".form-group").find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#Compra_serie").closest(".form-group").find("label").css("color", "red");
            valorretorno = false;
        }
        if($("#Compra_numero").val().trim()==""){
            $("#Compra_numero").closest(".form-group").find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#Compra_numero").closest(".form-group").find("label").css("color", "red");
            valorretorno = false;
        }
        ...
        if($("#Compra_idcliente").val().trim()==""){
            $("#Compra_idcliente").closest(".form-group").find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#Compra_idcliente").closest(".form-group").find("label").css("color", "red");
        ...
        $("#Compra_detalle").val(JSON.stringify(lista_det));
    ...
    function fn_confirmar_eliminar(id){
        ...
        function(isConfirm){
            if (isConfirm) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('compra.destroy', ':id') }}".replace(':id', item["id"]),
                    dataType: 'json',
                    data: {
                        id: item["id"],
                        _token: '{{ csrf_token() }}'
    ...
    function fn_editar(id){
        ...
        $("#esNuevo").val("2");
        $("#titulo_poput_mantenedor_emisor").html('<span class="glyphicon glyphicon-tasks"></span> Editar Compra #'+data.id);

        $("#compra-form").find('input').val("");
        ...
        $("#Compra_numero").val(data.numero || '');
        $("#Compra_serie").val(data.serie || '');
        $("#Compra_tipodoc").val(data.tipodoc || '');
        ...
        $("#Compra_idcliente").val(data.idcliente || '');
        $("#Compra_idmediopago").val(data.idmediopago || '');
        ...
        for(var p in data.detalles){
            $("#idtabla_productos").find("tbody").append(`
                <tr class="itemprecform">
                    ...
                    <td class="form-group input-group-sm"><input style="width:100%;text-align:center;" class="form-control calcdetform allow_decimal" type="number" value="${parseFloat(data.detalles[p]["precio"]).toFixed(2)}"></td>
                    ...
                    <td class="form-group input-group-sm">${(parseFloat(data.detalles[p]["precio"])*parseFloat(data.detalles[p]["cantidad"])).toFixed(2)}</td>
                    <td>
    ...
    function fn_cambiar_estado(ctrl,id){
        $.ajax({
            type: "POST",
            url: "route('compra.cambiar-estado') }}",
            dataType: 'json',
            data: {
                id: id,
                _token: '{{ csrf_token() }}'
