    ...
    <div class="col-md-12">
        <div class="box box-info">
            <div class="box-header with-border">
                <h4><i class="fa fa-users"></i> Gestión de clientes</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-md" role="document">
        <div class="modal-content" id="form_psasadsfd23423">
            ...
            <div class="modal-body">
                @include('cliente._form', ['model' => $model])
            </div>
...
<script type="text/javascript">
    var urlarea = "{{ route('cliente.index') }}";
    var ulrproj = "{{ url('/') }}";
    var listado = [];
    var tabla;
    ...
    $(document).ready(function (){
        ...
        $(".menu_clientes a").append(`<span style="color:yellow;" class=" pull-right"><i class="fa fa-check" aria-hidden="true"></i></span>`);
                ...
                for(var p in arreglo.lista){
                    $("#idtabla_listado tbody").append(`
                        <tr>
                            ...
                            <td>${arreglo.lista[p]["domicilio"]}</td>
                            <td>${arreglo.lista[p]["persona"]}</td>
                            <td>${arreglo.lista[p]["contacto"]}</td>
                            <td>${arreglo.lista[p]["correo"]}</td>
    ...
    function fn_nuevo(){
        ...
        $("#titulo_poput_mantenedor_emisor").html('<span class="fa fa-file"></span> Nuevo Cliente');

        $("#cliente-form").find('input').val("");
        $("#esNuevo").val("1");
    ...
    function fn_guardar(){
        if(fn_validar_ctrls()){
            ...
            var url1 = "{{ route('cliente.store') }}";
            var msjjj = "Cliente registrado";
            let tipo='POST';
            var formData = new FormData(document.forms.namedItem("cliente-form"));
            if($("#esNuevo").val()!="1"){
                url1 = "{{ route('cliente.update', ':id') }}".replace(':id', $("#Registro_id").val());
                msjjj = "Cliente actualizado";
                tipo='PUT';
            }
    ...
    function fn_validar_ctrls(){
        ...
        if($("#razonsocial").val().trim()==""){
            $("#razonsocial").parent().parent().find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#razonsocial").parent().parent().find("label").css("color", "red");
            valorretorno = false;
        }
        if($("#ruc").val().trim()==""){
            $("#ruc").parent().parent().find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#ruc").parent().parent().find("label").css("color", "red");
            valorretorno = false;
        }
    ...
    function fn_confirmar_eliminar(id){
        ...
        swal({
            ...
            text: "Desea eliminar al cliente "+item["nombre"]+"?",
            type: "info",
        ...
        function(isConfirm){
            if (isConfirm) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('cliente.destroy', ':id') }}".replace(':id', item["id"]),
                    dataType: 'json',
                    data: {
                        id: item["id"],
                        _token: '{{ csrf_token() }}'
    ...
    function fn_editar(id){
        ...
        $("#esNuevo").val("2");
        $("#titulo_poput_mantenedor_emisor").html('<span class="glyphicon glyphicon-tasks"></span> Editar Cliente');
    ...
    function fn_cambiar_estado(ctrl,id){
        $.ajax({
            type: "POST",
            url: "route('cliente.cambiar-estado') }}",
            dataType: 'json',
            data: {
                id: id,
                _token: '{{ csrf_token() }}'
