    ...
    <div class="modal-dialog modal-dialog-centered modal-md" role="document">
        <div class="modal-content" id="form_psasadsfd23423">
            ...
            <div class="modal-body">
                @include('categoria._form', ['model' => $model])
            </div>
...
<script type="text/javascript">
    var urlarea = "{{ route('categoria.index') }}";
    var ulrproj = "{{ url('/') }}";
    var listado = [];
    var tabla;

    $(document).ready(function (){
        $(".menu_categorias").addClass("active");
        $(".menu_categorias a").append(`<span style="color:yellow;" class=" pull-right"><i class="fa fa-check" aria-hidden="true"></i></span>`);
    ...
    function fn_nuevo(){
        ...
        $("#titulo_poput_mantenedor_emisor").html('<span class="fa fa-file"></span> Nueva Categoría');
    ...
    function fn_guardar(){
        if(fn_validar_ctrls()){
            ...
            var url1 = "{{ route('categoria.store') }}";
            var msjjj = "Categoría registrada";
            let tipo='POST';
            var formData = new FormData(document.forms.namedItem("carro-form"));
            if($("#esNuevo").val()!="1"){
                url1 = "{{ route('categoria.update', ':id') }}".replace(':id', $("#Registro_id").val());
                msjjj = "Categoría actualizado";
                tipo='PUT';
            }
    ...
    function fn_validar_ctrls(){
        ...
        if($("#categoria").val().trim()==""){
            $("#categoria").parent().parent().find("label").append(' <i rel="tooltip" data-animate="animated fadeIn" data-toggle="tooltip" data-original-title="Ingrese un nombre" class="fa fa-exclamation-circle"></i>');
            $("#categoria").parent().parent().find("label").css("color", "red");
            valorretorno = false;
        }
    ...
    function fn_confirmar_eliminar(id){
        ...
        swal({
            ...
            text: "Desea eliminar la categoría "+item["categoria"]+"?",
            type: "info",
        ...
        function(isConfirm){
            if (isConfirm) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('categoria.destroy', ':id') }}".replace(':id', item["id"]),
                    dataType: 'json',
                    data: {
                        id: item["id"],
                        _token: '{{ csrf_token() }}'
    ...
    function fn_editar(id){
        ...
        $("#esNuevo").val("2");
        $("#titulo_poput_mantenedor_emisor").html('<span class="glyphicon glyphicon-tasks"></span> Editar Categoría');
    ...
    function fn_cambiar_estado(ctrl,id){
        $.ajax({
            type: "POST",
            url: "route('categoria.cambiar-estado') }}",
            dataType: 'json',
            data: {
                id: id,
                _token: '{{ csrf_token() }}'
